...
def createTable(cursor, tableName, columns):
    ...
    my_query= "DROP TABLE IF EXISTS {}".format(tableName)
    cursor.execute(my_query)

    my_query= "CREATE TABLE {} ({})".format(tableName,columns)
    cursor.execute(my_query)

    print("tabl created {}".format(tableName))

def insertValues(cursor, tableName, valueString):
    """
    Given the database cursor, the table name, and the values to add
    """
    the_query="INSERT INTO {} VALUES ({})".format(tableName,valueString)
...
def queryStatement(theSelect, theFrom, theWhere=""):
    ...
    """
    the_query= "SELECT {} FROM {}".format(theSelect, theFrom)
    if theWhere != "":
        the_query+= " WHERE {}".format(theWhere)
...
if __name__ == "__main__":
    path = "C200-Assignments-pmanolis/Laboratory/Lab11/ourDB.db"
    ...
    query1= queryStatement("Owner.Name, Animal.Name", "Owner, Animal", "Owner.AnimalID == Animal.ID and Animal.Type == 'Cat'")

    query = queryStatement("", "", "") 
    for x in theCursor.execute(query1):
        print(x)
    ...
    query2 = queryStatement("Owner.Name, Animal.Name", "Owner, Animal", "Owner.AnimalID == Animal.ID and Animal.Age <= 3")
    query = queryStatement("", "", "") 
    for x in theCursor.execute(query2):
        print(x)
    ...
    query3 = queryStatement("Count(distinct Vet.VetID)", "Vet, Owner o1, Owner o2", "Vet.VetID == o1.VetID and Vet.VetID == o2.VetID and o1.Name!=o2.Name") 
    for x in theCursor.execute(query3):
        print(x)
